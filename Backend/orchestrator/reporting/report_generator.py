...
def _render_plan_section(test_case, result: RunResult) -> tuple[str, list]:
    """Return (plan_html, checkpoints_list) for the test plan section."""
    plan_html = ""
    checkpoints = []
    if not (test_case and test_case.goal):
        return plan_html, checkpoints
...
def _render_assertions_html(result: RunResult) -> str:
    """Return the assertions <h2> + <table> block, or empty string if no assertions."""
    assertion_rows = ""
...
def _render_step_card(t, cp_lookup: dict) -> str:
    """Return the HTML for a single step card."""
...
def _report_css(status_color: str) -> str:
    """Return the full CSS block for the HTML report."""
    return f"""
...
def generate_html_report(
    ...
    *,
    ...
    plan_html, checkpoints = _render_plan_section(test_case, result)
    cp_lookup = {cp.id: cp for cp in checkpoints}
    assertions_section = _render_assertions_html(result)
    step_cards = "".join(_render_step_card(t, cp_lookup) for t in traces)
...
<style>{_report_css(status_color)}</style>
</head>
<body>
